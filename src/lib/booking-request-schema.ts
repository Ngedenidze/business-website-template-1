...
export const bookingRequestSchema = z
  .object({
    ...
    selectedDeliveryDistance: z.string().trim().max(80).optional().or(z.literal("")),
    selectedSetupTent: z.string().trim().max(80).optional().or(z.literal("")),
    selectedAddOns: z.array(z.string().trim().min(1).max(160)).max(25).optional(),
    estimatedPackagePrice: z.number().min(0).optional(),
    estimatedAddOnPrice: z.number().min(0).optional(),
    estimatedDeliveryFee: z.number().min(0).optional(),
    estimatedSetupFee: z.number().min(0).optional(),
    estimatedTotal: z.number().min(0).optional(),
    matchedServiceTown: z.string().trim().max(120).optional(),
    matchedTownDistanceMiles: z.number().min(0).optional(),
    matchedDeliveryTier: z.string().trim().max(120).optional(),
  ...
  .transform((payload) => ({
    ...
    selectedDeliveryDistance: payload.selectedDeliveryDistance || undefined,
    selectedSetupTent: payload.selectedSetupTent || undefined,
